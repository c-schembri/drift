...
Drift v0 package support intentionally has no registry, version solver, transitive package graph, binary package format, or package action targets. A package must be pinned to an exact Git commit or archive digest and expose a native Drift project or a trusted local overlay. Drift does not translate its graph into an IDE's native build engine, sandbox commands, or perform cross-compilation in v0. Visual Studio generation emits Makefile-style projects backed by Drift and Ninja rather than a second MSBuild backend. Remote execution never occurs implicitly during a local build.
